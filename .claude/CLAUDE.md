...
│       └── dev/                # 개발용 테스트 API
├── components/
│   ├── ui/                     # PixelFrame, PixelButton, StatBar, ElementTag, PixelSelect, PixelDivider
...
├── lib/
│   ├── ai/                     # Gemini 클라이언트 (9종 프롬프트), 스탯 스코어러, HTML 새니타이저
...
| 로그인 + 캐릭터 있음 | `/login`, `/onboarding` | → `/` (허브) |
...
characters: id, user_id, name, birth_date, birth_time, birth_city, gender, mbti, is_self, unlocked
...
### 미완료 (우선순위순)
- [ ] Toss Payments 실제 연동 (위젯 + 실제 키)
- [ ] Vercel 배포 (환경변수, 도메인)
- [ ] 네이버/구글 OAuth 설정
- [ ] 비로그인 마케팅 랜딩 페이지
- [ ] 캐릭터 픽셀 아바타 이미지 10종 제작
- [ ] SNS 공유 (결과 이미지 생성)
- [ ] 스킬트리 연결선 CSS (2차)
- [ ] 캐릭터 삭제/편집 기능
