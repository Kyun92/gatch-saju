...
- 비즈니스 모델: 무료(일일운세+캐릭터생성) → 코인 충전(990원/개) 후 운세 1건당 코인 1개 소비 (박리다매, 9종 상품)
- 핵심 컨셉: **1메인(본인) + N타인(가족·친구)** — 본인은 메인 프로필로 항상 강조되고, 추가 캐릭터는 가족·친구의 사주를 RPG 캐릭터처럼 관리한다. 본인 캐릭터는 user당 1명만 존재(onboarding/actions.ts 가드)하며, 마이페이지/허브에서 시각적으로 우선 노출된다. OAuth 프로필 이미지는 사용하지 않고, 본인 캐릭터의 픽셀 아바타가 메인 프로필을 대체한다.
...
### 감정 생성 비동기 패턴
- 운세 뽑기 요청 → 코인 잔액 확인 (부족 시 402 + `INSUFFICIENT_BALANCE_MESSAGE` → `/coins?returnTo=...`)
- 잔액 충분 → 코인 1개 차감(RPC) → readings INSERT (status='pending') → Background에서 생성
- 클라이언트 3초 폴링 → 완료 시 결과 페이지
- 코인 충전(`/coins`)은 별도 Toss 결제 흐름: Toss 결제 → success 페이지 → 결제 승인 API → users.coins 증가 → returnTo로 복귀
...
### 캐릭터 시스템
- **본인 1명(메인) + 타인 N명** — `characters.is_self`로 분류. 본인 캐릭터는 user당 1명만 존재(onboarding 가드 + 코드 흐름으로 보장). 마이페이지/허브에서 시각적으로 우선 노출
- 오행 5종 × 성별 2종 = 10종 프리셋
- 레벨 = 만 나이
- 아바타 경로: `/characters/{element}-{gender}.png` — element는 `getCharacterElement(dayMaster)` 결정론 매핑 사용 (fiveElements maxEntry 금지)
- 프리셋 매핑: 일간(日干) → 오행 자동 결정
- 스킬트리: 종합감정(루트) → 년운/연애운/직업운/금전운/건강운/학업운/궁합(가지)
- **호칭 분기**: 외곽 헤더는 `getVocative`/`getPossessive`/`getHeaderTitle` (`@/lib/copy/character-vocative`) 사용 — 본인 "당신" / 타인 "{이름}님". LLM HTML 본문은 변경 금지
...
**목표:** 신규 기능(페이지·API·상품·차트 항목)을 DB → API → UI → QA 파이프라인으로 일관되게 추가한다. 모든 변경에서 1계정 N캐릭터 소유권, 결제 해금 전제, DOMPurify, 픽셀 RPG CSS 컨벤션을 자동 강제한다.

**트리거:** 풀스택 변경이 필요한 작업 요청 시 `feature-orchestrator` 스킬을 사용하라. 단순 질문, 단일 파일 수정, 코드 리뷰는 직접 응답 가능.

**팀:** feature-planner → db-migrator → api-builder → ui-builder → integration-qa (5인, 점진적 QA)

**변경 이력:**
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-04-28 | 초기 구성 | 전체 | 신규 기능 추가 풀스택 워크플로우 자동화 |
| 2026-04-28 | 가격·캡슐·코인 카피 단일화 | ui/api + CLAUDE.md | 결제 모델 표현 분열 통합 (990원 = 1코인 = 캡슐 1개 단일 토큰, gacha-terms.ts) |
| 2026-04-28 | 한자 직접 노출 제거 + 비유 체계 + element 일관화 | ui + lib/copy/day-master.ts | P0 한자 노출 0건화, dayMaster↔element 결정론 매핑 헬퍼 신설로 fiveElements maxEntry 재계산 버그 3곳 제거 |
...
| 2026-04-29 | SNS 공유(OG 이미지·ShareButtons) + 인라인 style 1차 마이그레이션 | api/og/reading/[id] + ShareButtons + ReadingCTA + reading/[id] generateMetadata + middleware + PixelDivider/PixelSelect/global-error/landing/login + globals.css | next/og 1200×630 RPG 카드(무인증 화이트리스트), Web Share API + clipboard fallback, og:image+twitter meta 동적 주입, /api/og STATIC_PATHS 추가, globals.css Lane A 클래스 200줄+ 추가 + 5개 컴포넌트 적용 (UI 코어 일부·landing/login/global-error 인라인 제거) |
| 2026-04-29 | 인라인 style 2차 마이그레이션 (UI 코어 + layout) | PixelButton/PixelFrame/StatBar/ElementTag/FourElementFusion/layout.tsx + globals.css (color-scheme-light, four-element-svg) | UI 코어 정적 인라인 -7곳, FourElementFusion SVG -4곳·width/height -1, layout color-scheme/배경/폰트 -2. 누적 -43건 마이그레이션, 잔여 ~35건은 동적 props 합법 인라인 (영향도 분석 분류 일치) |
| 2026-04-29 | NAVER/GOOGLE OAuth 활성화 | lib/auth.ts + login/page.tsx + LoginModal.tsx + docs/vercel-env-setup.md | 조건부 Provider 등록(env 누락 시 미등록 — 런타임 에러 방지), non-null assertion 제거, "준비 중" UI 제거 + signIn 활성화. 환경변수 가이드 NAVER/GOOGLE 발급처·Redirect URI 추가 |
| 2026-04-29 | .env 템플릿 양식 통일 | .env.local.example + .env.vercel.example | 카테고리 6개(Supabase/NextAuth/공개 URL/OAuth/Gemini/Toss) 동일 구조로 통일, NAVER/GOOGLE 누락 보강, NEXT_PUBLIC_SITE_URL 추가, 미사용 TOSS_CLIENT_KEY 제거. local은 USE_MOCK_READINGS=true 포함, vercel은 절대 금지 경고만 |
| 2026-04-29 | NAVER/GOOGLE UI 출시 1차 비활성 | login/page.tsx + LoginModal.tsx + docs/vercel-env-setup.md + .env.example + Obsidian 가이드 | 네이버 검수·구글 OAuth 검토 시간으로 출시 지연 회피. UI 두 버튼 제거(코드는 주석 + 활성화 절차 명시). auth.ts 조건부 Provider는 그대로 보존(env 비어 있으면 자동 미등록 안전망). 가이드/템플릿 모두 "출시 1차 비활성" 표기 |
| 2026-05-08 | 보안 감사 CRITICAL/HIGH 4건 코드 패치 | api/reading/[id] + api/dev/test-daily + supabase/schema.sql + supabase/migrations + next.config.ts | C-3 reading IDOR 회귀 차단(존재하지 않는 user_id 컬럼 → character 조인으로 소유권 검증). C-4 dev 라우트 production 404 + auth + 소유권 이중 가드. C-2 RLS ENABLE 9개 테이블 + REVOKE anon/authenticated. H-6 보안 헤더(CSP·X-Frame-Options·X-Content-Type-Options·Referrer-Policy·Permissions-Policy). C-1 키 로테이션·NEXTAUTH_SECRET 재생성은 사용자 작업 |
